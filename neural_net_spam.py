...
print(training_spam.shape)

class spamClassifier:
    def __init__(self, layer_descriptions = [54, 25, 1], weights_biases = {}):
    ...
    def initilaize_weights_biases(self): # Randomly initialize weights and biases
    ...
    def forward_propagation(self, X):
        ...
        for layer in range(1, no_layers):
            W, b = self.weights_biases[f"W{layer}"], self.weights_biases[f"b{layer}"]
            Z = np.dot(W, A) + b # hidden layer linear transformation (applying weights and biases)
            A, inputted_Z = self.ReLU(Z) # hidden layer activation (ReLU)
            old_values.append((A, W, b, inputted_Z, 'relu'))
        
        W, b = self.weights_biases[f"W{no_layers}"], self.weights_biases[f"b{no_layers}"]
        Z = np.dot(W, A) + b # output layer linear transformation (applying weights and biases)
        A, inputted_Z = self.sigmoid(Z) # output layer activation (sigmoid)
        old_values.append((A, W, b, inputted_Z, 'sigmoid'))
        return A, old_values
    ...
    def backward_propagation(self, last_A, Y, old_values):
        ...
        prev_A, W, _, old_Z, _ = old_values[no_layers - 1]
        dZ = last_A - Y # derivative of cost function with respect to last_A (accuracy)
        gradients[f"dW{no_layers}"] = (1/no_sets) * np.dot(dZ, prev_A.T) # gradient of cost function with respect to W
        gradients[f"db{no_layers}"] = (1/no_sets) * np.sum(dZ, axis=1, keepdims=True) # gradient of cost function with respect to b

        prev_dA = np.dot(W.T, dZ) # gradient of cost function with respect to A (previous layer)

        # hidden layer(s)
        for layer in reversed(range(no_layers - 1)):
    ...
    def train(self, X, Y, learning_rate = 0.01, iterations = 1000, print_cost = False):
        ...
        self.initilaize_weights_biases()
    ...
    def predict(self, X):
        ...
        return prediction > 0.5 # return 1 if prediction is greater than 0.5 else 0
